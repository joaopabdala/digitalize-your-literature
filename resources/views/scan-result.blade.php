        ...
        <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-4xl space-y-6">
            ...
            <div class="flex flex-wrap -mx-4">
                <div class="w-full md:w-1/2 px-4 mb-6 md:mb-0">
                    ...
                    @if(isset($page['imageUrl']))
                        <img src="{{$page['imageUrl'] }}" alt="Scanned Image" class="rounded-lg shadow-md max-w-full h-auto">
                    @else
                    ...
                    <div id="formattedContent" class="prose max-w-none overflow-auto max-h-[600px]">
                        @if(isset($page['pageData']))
                            @if(!empty($page['pageData']['headerTitle']))
                                <h1 class="text-4xl font-extrabold mb-4">{!! $page['pageData']['headerTitle'] !!}</h1>
                            @endif
                            @if(!empty($page['pageData']['title']))
                                <h2 class="text-3xl font-bold mb-3">{!! $page['pageData']['title'] !!}</h2>
                            @endif
                            @if(!empty($page['pageData']['subtitle']))
                                <h3 class="text-xl font-semibold text-gray-600 mb-6">{!! $page['pageData']['subtitle'] !!}</h3>
                            @endif
                            @if(!empty($page['pageData']['paragraphs']) && is_array($page['pageData']['paragraphs']))
                            ...
                            @if(!empty($page['pageData']['pageNumber']))
                                <p class="text-sm text-right mt-6">Page: {!! $page['pageData']['pageNumber'] !!}</p>
                            @endif
                        @else
                ...
                <button onclick="copyPlainText('plainTextContent-{{ $index }}')"
            ...
            <div class="flex justify-center mb-6 gap-x-4">
                ...
                @auth
                    ...
                    <form action="{{route('digitalize.destroy', ['digitalizationBatch' => $digitalizationBatch->id])}}" method="post">
                        @csrf
                        @method('delete')
                        <button
                            type="submit"
