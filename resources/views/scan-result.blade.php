            ...
            @foreach($pages as $index => $page)
                <div class="flex flex-wrap -mx-4">
                    <div class="w-full md:w-1/2 px-4 mb-6 md:mb-0">
                        ...
                        @if(isset($page['imageUrl']))
                            <img src="{{$page['imageUrl'] }}" alt="Scanned Image"
                ...
                <div class="flex justify-end gap-x-2">
                    ...
                    <button onclick="copyPlainText('plainTextContent-{{ $index }}')"
                        ...
                        <svg id='Copy_to_Clipboard_24' width='24' height='24' viewBox='0 0 24 24'
                    ...
                    <textarea id="plainTextContent-{{ $index }}"
                              class="hidden">{{ $page['plainText'] ?? '' }}</textarea>

                    <form method="POST"
                          action="{{route('reDigitalize', ['digitalization' => $page['digitalization_id']])}}">
                        @csrf
                        @method('POST')
                        <button
                            class="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-lg shadow-md transition duration-200 flex items-center">
                            Re-process {{$page['digitalization_id']}}
                        </button>
                    </form>
                </div>
